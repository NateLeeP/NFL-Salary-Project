...
class Quarterback():
    ...
    fb_url = 'https://www.pro-football-reference.com/players/{}/{}00.htm'
    sp_url = 'https://www.spotrac.com/nfl/{}/{}'
    def __init__(self, name, fb_url = None,sp_url = None):
        self.name = name
        #scrape team name and stats at same time, so as not to make multiple requests
        if fb_url is not None:
            self.pro_fb_url = fb_url
        else:
            self.pro_fb_url = self.fb_url.format(name.split()[1][0], name.split()[1][:4] + name.split()[0][:2])
        ...
        if sp_url is not None:
            ...
            self.spotrac_url = self.sp_url.format(self.team_name.lower().replace(' ','-'), name.lower().replace(' ','-'))
    ...
    def scrape_qb_data_pro_football(self, name):
        ...
        # Pattern to account for players with 'II' after their name. Will match on first name last name. 
        pattern = re.compile(r'\w+ \w+')
        url = self.pro_fb_url
        # Scrape QB data. Return Soup
        qb_request = requests.get(url).text
        qb_soup = bs(qb_request, "lxml")
        if pattern.search(qb_soup.find('h1', {'itemprop':'name'}).text.strip()) == name:
            qb_soup = qb_soup
        else:
...
hot_keys = {'Josh Allen':['https://www.pro-football-reference.com/players/A/AlleJo02.htm','https://www.spotrac.com/nfl/buffalo-bills/josh-allen-25102/'],
            'Daniel Jones':['https://www.pro-football-reference.com/players/J/JoneDa05.htm','https://www.spotrac.com/nfl/new-york-giants/daniel-jones-29041/']}
